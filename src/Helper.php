...
class Helper {
	...
	public static function decodeValue($value, $type) {
		...
		switch($type) {
			...
			case 'interval' :
				preg_match('~^(?:(?P<y>[-0-9]+)? years?\s?)?(?:(?P<m>[-0-9]+)? mons?\s?)?(?:(?P<d>[-0-9]+)? days?\s?)?(?:(?P<h>[-0-9]{1,3})?:(?P<i>[-0-9]{1,3})?:(?P<s>[-0-9]{1,3})?)?$~', $value, $matches);
				$matches = array_merge(['y' => 0, 'm' => 0, 'd' => 0, 'h' => 0, 'i' => 0, 's' => 0], $matches);
				$format = sprintf('P%dY%dM%dDT%dH%dM%dS', $matches['y'], $matches['m'], $matches['d'], $matches['h'], $matches['i'], $matches['s']);
